...
def main():
    ...
    subprocess.run(["rm", "-f", "/tmp/zs_*.log"])
    subprocess.run(["rm", "-f", "/tmp/benchmark*.json"])
    subprocess.run(["rm", "-f", "/tmp/zs_*.json"])
    subprocess.run(["rm", "-f", "/tmp/*.prof"])

    run_on_nodes(args, " ".join(["rm", "-f", "/tmp/zs_*.log"]))
    ...
    transfer_files_from_nodes(args, "/tmp/zs_*.log", "/tmp/", recursive=False)
...
def run_experiments_on_nodes(args):
    ...
    for i in range(total_container):
        ...
        cmd = f"run_exp -m 'modubft' -n 0 -- ./cli  --id {i} --send_transaction {args.send_transaction} --cluster_prepare_to_all_and_enable_hash {args.cluster_prepare_to_all_and_enable_hash} --buckets {args.buckets} --alternateClusterPrepare {args.alternateClusterPrepare}  --nodes {args.ips}   --roles {args.roles}   --clusters {args.cluster}   --dont_verify_cluster_commit {args.dont_verify_cluster_commit}   --use_pk_to_client {args.use_pk_to_client}   --profile {args.profile}   --runtime {args.runtime}   --vallen {args.vallen}   --cluster_batch_size {args.cluster_batch_size} --use_dool {args.use_dool} --use_certificate_compression {args.use_certificate_compression} 2>&1 | tee -a  /tmp/zs_{i}.log &"
    ...
    for idx_ssh_ip, cmd in cmds.items():
        ...
        ssh_command = f"ssh -o StrictHostKeyChecking=no {args.username}@{idx_ssh_ip} 'bash -s' <<EOF\n{cmds[idx_ssh_ip]}\nEOF"
        ...
        #     f"{args.username}@{idx_ssh_ip}", "<<", cmds[idx_ssh_ip]
        # ], shell=True)
        # proc.stdout
        # print(f"Started process on {idx_ssh_ip} with PID {proc.pid}")
        # get output
        # stdout, stderr = proc.communicate()
        # print(f"Output from {idx_ssh_ip}:\n{stdout.decode()}")
        # if stderr:
        #     print(f"Error from {idx_ssh_ip}:\n{stderr.decode()}")
        pids.append(proc)

    def stream_output(proc, idx_ssh_ip):
        for line in proc.stdout:
            print(f"[{idx_ssh_ip}] {line.strip()}", flush=True)

    threads = []
    for proc, idx_ssh_ip in zip(pids, cmds.keys()):
        t = threading.Thread(target=stream_output, args=(proc, idx_ssh_ip))
        t.daemon = True
        t.start()
        threads.append(t)

    wait_for_process_completion(pids, pipe=False)

    print("All processes completed.", flush=True)


def setup_node_environment(args):
    transfer_files_to_nodes(args, "/tmp/cli", "~/")
    ...
    for i in range(total_container):
        ...
        if add_idx:
            ...
            proc_args.extend([
                ...
                f"{file_to_copy_dst}cpu_{i}.prof"
            ])
        else: 
...
def wait_for_process_completion(pids, pipe=False):
    for proc in pids:
