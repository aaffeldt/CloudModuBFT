...
#   availability_zone = vpc.module.controller_az
#   private_ip        = vpc.module.controller_ip
# # }
variable "availability_zone" {
  description = "The availability zone where the control instance will be deployed"
  type        = string

}

variable "private_ip" {
  description = "The private IP address to assign to the control instance"
  type        = string
...
resource "aws_instance" "control_instance" {
  ...
  private_ip    = var.private_ip                                  # Assign a private IP within the VPC CIDR block
  ...
  subnet_id         = var.subnet             # Specify the subnet for the instance
