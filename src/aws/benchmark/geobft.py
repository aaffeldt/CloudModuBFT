...
def main():
    ...
    for number_of_cluster in [6,2]:
        for number_of_peers in [6,3]:
            batch_sizes = [1,128]
            run_benchmark(basefolder, number_of_peers, number_of_cluster,batch_sizes)
    
def run_benchmark(basefolder, number_of_peers, number_of_cluster,batch_sizes):
    print(
    ...
    cmd = create_script_execution_command(
        basefolder, number_of_peers, number_of_cluster,batch_sizes, script_dir)
...
def create_script_execution_command(basefolder, number_of_peers, number_of_cluster,batch_sizes, script_dir):
    cmd = ["pushd", f"'{script_dir}'", "&&"] + \
        generate_benchmark_command(
            basefolder, number_of_peers, number_of_cluster,batch_sizes) + ["&&", "popd"]
    cmd = " ".join(map(str, cmd))
    cmd = ["bash", "-c", cmd]
    return cmd


def generate_benchmark_command(basefolder, number_of_peers, number_of_cluster,batch_sizes):
